    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        let filteredMovies = data.results.filter(movie => {
          ...
          if (releaseYear && !movie.release_date.includes(releaseYear)) return false;
          if (rating) {
            if (ratingComparison === 'equal' && movie.vote_average !== parseFloat(rating)) return false;
            if (ratingComparison === 'higher' && movie.vote_average <= parseFloat(rating)) return false;
            if (ratingComparison === 'lower' && movie.vote_average >= parseFloat(rating)) return false;
          }
          return true;
        });
